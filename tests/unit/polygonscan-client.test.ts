import { PolygonscanClient, createPolygonscanClient } from '../../src/services/polygonscan-client';
...
describe('PolygonscanClient', () => {
  ...
  describe('Constructor and Configuration', () => {
    ...
    it('should initialize with custom network', () => {
      const mumbaiClient = createPolygonscanClient({
        network: 'mumbai'
      });
      expect(mumbaiClient.getNetwork()).toBe('mumbai');
    });

    it('should throw error for unsupported network', () => {
      expect(() => {
        new PolygonscanClient({
          baseUrl: '',
      ...
      }).toThrow('Unsupported Polygon network: invalid');
    });
  });

  describe('Address Validation', () => {
    it('should validate valid Polygon addresses', async () => {
      const validAddress = '0x1234567890123456789012345678901234567890';
      
      // Should not throw for valid address
      expect(() => {
        (client as any).validatePolygonAddress(validAddress);
      }).not.toThrow();
    });
    ...
    it('should reject invalid Polygon addresses', () => {
      ...
      invalidAddresses.forEach(address => {
        expect(() => {
          (client as any).validatePolygonAddress(address);
        }).toThrow();
      });
    });
  });
  ...
  describe('Network URL Mapping', () => {
    it('should return correct URLs for different networks', () => {
      expect((PolygonscanClient as any).getBaseUrl('mainnet')).toBe('https://api.polygonscan.com/api');
      expect((PolygonscanClient as any).getBaseUrl('mumbai')).toBe('https://api-testnet.polygonscan.com/api');
    });
  });

  describe('Factory Function', () => {
    it('should create client with default configuration', () => {
      const defaultClient = createPolygonscanClient({});
      expect(defaultClient.getNetwork()).toBe('mainnet');
    });

    it('should create client with custom configuration', () => {
      const customClient = createPolygonscanClient({
        network: 'mumbai',
        apiKey: 'test-key',
        rateLimit: {
      ...
      expect(customClient.getNetwork()).toBe('mumbai');
    });
  });
